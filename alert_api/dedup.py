...
class DedupFilter:
    ...
    otherwise defeat a plate-only dedup key:
      - Clean, regex-valid plate -> key = (plate, camera_id). Same vehicle
        keeps suppressing duplicates for as long as it keeps reappearing
        within the window, regardless of exact timing.
      - Empty/garbled plate (OCR noise, "UNREAD") -> key = (camera_id, time_bucket).
